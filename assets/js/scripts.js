...
highlightRightSidebar = function() {
  ...
  document
    .querySelectorAll(".c-textbook__content h2, .c-textbook__content h3")
    .forEach((header, index) => {
      var target = header.offsetTop;
      var id = header.id;
      if (position >= target) {
        var query = 'ul.toc__menu a[href="#' + id + '"]';
        document.querySelectorAll("ul.toc__menu li").forEach(item => {
  ...
  document
    ...
    .addEventListener("scroll", highlightRightSidebar);
};
